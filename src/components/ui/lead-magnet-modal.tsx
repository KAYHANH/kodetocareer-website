...
export default function LeadMagnetModal({ isOpen, onClose, resourceTitle = '2026 Tech Career & Interview Roadmap PDF' }: LeadMagnetModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-slate-900 border border-slate-800 rounded-3xl p-5 sm:p-8 max-w-md w-full relative shadow-2xl space-y-5">
        ...
        <div className="space-y-1">
          ...
          <p className="text-xs text-slate-300 font-medium">{resourceTitle}</p>
        </div>

        {error && (
          <div className="p-3.5 rounded-xl bg-rose-950/80 border border-rose-500/30 text-rose-200 text-xs font-bold">
            {error}
          </div>
        )}
        ...
        {downloaded ? (
          <div className="bg-emerald-950/40 border border-emerald-500/30 p-6 rounded-2xl text-center space-y-4">
            ...
            <button
              onClick={onClose}
              className="w-full h-[50px] bg-slate-800 hover:bg-slate-700 text-white font-extrabold text-xs rounded-xl"
            >
              Close
            </button>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input
                ...
                className="w-full h-[50px] bg-slate-950 border border-slate-800 rounded-xl px-4 text-xs font-semibold text-white placeholder:text-slate-500 focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/20"
              />
            </div>
            ...
            <div>
              <label className="text-xs font-bold text-slate-200 block mb-1">Phone / WhatsApp Number *</label>
              <input
                type="tel"
                required
                value={phone}
            ...
            <button
              type="submit"
              disabled={submitting}
              className="w-full h-[50px] bg-primary hover:bg-blue-600 text-white font-black text-xs sm:text-sm rounded-xl shadow-lg transition-colors flex items-center justify-center gap-2 cursor-pointer mt-2"
            >
